...
def noise(D_file: str, f_file: str, regime: str, bval_file: str, 
          noise_sigma: float, outbase: str, n_noise: int = 1, S0_file: str | None = None, 
    ...
    n1 = np.random.normal(loc=0,scale=noise_sigma,size=Y.shape[:-1]+(n_noise,Y.shape[-1]))
    n2 = np.random.normal(loc=0,scale=noise_sigma,size=Y.shape[:-1]+(n_noise,Y.shape[-1]))
    Ynoise = np.sqrt((np.expand_dims(Y,axis=-2)+n1)**2 + n2**2)
